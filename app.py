...
def call_google(query):
    ...
    if not key: return None, None, "API Key Missing in Secrets"
    ...
    try:
        ...
        return None, None, status # Returns: REQUEST_DENIED, ZERO_RESULTS, etc.
...
t_col.markdown("### GoGeoCoder <span style='font-size:14px; color:gray;'>v1.3</span>", unsafe_allow_html=True)

# Row 2: Menu and Counters (Compact)
m_col, c_col = st.columns([1.8, 1.2])
with m_col:
    menu = st.selectbox("Menu", ["Import XLSX", "Export Success CSV", "Export Fail Log", "Reset System"], label_visibility="collapsed")
...
elif menu == "Export Fail Log":
    if not st.session_state.fails.empty:
        st.write("### Latest API Status:", st.session_state.fails['Reason'].iloc[-1] if not st.session_state.fails.empty else "N/A")
